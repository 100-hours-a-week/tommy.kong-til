...
**grep "검색어" <파일명>** 특정 파일에서 특정 단어 검색(*Global Regular Expression Print*)

**grep -r "검색어" <폴더>** 특정 폴더에서 모든 파일을 대상으로 검색

**whoami** 현재 로그인한 사용자 확인

**uname -a** 시스템 정보 출력 (운영체제, 커널 버전 등) (*unix name*)

**top** 실시간 CPU, 메모리 사용량 및 프로세스 확인(*table of processes*)

**ps aux** 실행 중인 프로세스 목록 확인 (*process status*)

**kill <PID> 특정 프로세스 종료 (PID는 ps aux로 확인 가능)

**kill -9 <PID>** 강제 종료 (SIGKILL)

**df -h** 디스크 사용량 확인(*disk free*)

**du -sh** 현재 폴더의 하위 파일/폴더 크기 확인 (*disk usage*)

**ping <도메인>** 특정 서버에 핑(응답 시간) 테스트(*Packet Internet Groper*)

**curl <URL>** 특정 URL의 내용 요청 (*client url*)

**wget <URL>** 특정 파일 다운로드 (*web get*)

**ifconfig** 또는 ip a 네트워크 인터페이스 및 IP 확인 (Linux) (*Interface Configuration*)

**ipconfig** 네트워크 정보 확인 (Windows) (*IP Configuration)

**netstat -tulnp** 현재 열려 있는 포트 확인 (*Network Statistics*)

**chmod 755 <파일>** 파일 권한 변경 (rwxr-xr-x) (*change mod*)

**chown 사용자:그룹 <파일>** 파일 소유권 변경 (*change owner*)

**sudo <명령어>** 관리자 권한으로 명령 실행 (*super user do*)

**su <사용자>** 다른 사용자 계정으로 변경 (*switch user*)

**tar -cvf archive.tar <파일/폴더>** tar 압축 파일 생성 (*tape archive*)
...
**fg %1** 백그라운드에서 실행 중인 작업을 포그라운드로 전환 (*foreground*)

**bg %1** 일시 정지된 작업을 백그라운드에서 다시 실행 (*background*)
